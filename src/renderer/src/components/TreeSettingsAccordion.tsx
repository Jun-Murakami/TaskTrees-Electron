...
export function TreeSettingsAccordion({ deleteTree }: TreeSettingsAccordionProps) {
  ...
  const handleAddUserToTree = async () => {
    ...
    try {
      ...
      await showDialog(
        'メンバーの追加に失敗しました。メールアドレスを確認して再度実行してください。' + error,
        'IInformation'
      )
        ...
        >
          <Stack direction="row" sx={{ height: 40, width: '100%', margin: '0 auto' }}>
            <img
              src={TaskTreesLogo}
              alt="Task Tree"
              style={{ width: '28px', height: '28px', marginTop: 5 }}
            />
            {isAccordionExpanded ? (
              <TextField
                id="outlined-basic"
                ...
                <React.Fragment key={index}>
                  ...
                  <ListItem disablePadding>
                    ...
                    >
                      <ListItemIcon>
                        <HighlightOffIcon
                          onClick={() => handleDeleteUserFromTree(member.uid, member.email)}
                        />
                      </ListItemIcon>
                    </ListItemButton>
                  </ListItem>
