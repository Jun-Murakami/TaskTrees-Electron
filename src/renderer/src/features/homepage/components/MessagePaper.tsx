...
export const MessagePaper = () => {
	...
	useEffect(() => {
		...
		const fetchLatestVersion = async () => {
			try {
				const repoUrl =
					"https://api.github.com/repos/Jun-Murakami/TaskTrees-Electron/releases/latest";
	...
	useEffect(() => {
		...
		if (currentVersion && latestVersion) {
			const currentVersionArray = currentVersion
				.split(".")
				.map((v) => parseInt(v));
			const latestVersionArray = latestVersion
				.split(".")
				.map((v) => parseInt(v));
	...
	return (
		<Stack sx={{ width: "100%", maxWidth: 400, margin: "auto", marginTop: 4 }}>
			...
			{isElectron && (
				<Paper
					sx={{ width: "100%", maxWidth: 400, margin: "auto", marginTop: 4 }}
				>
					<Typography
						variant="body2"
						sx={{ textAlign: "left", p: 2 }}
			...
			<Typography
				variant="caption"
				sx={{ width: "100%", minWidth: "100%", mt: 1, textAlign: "center" }}
			>
				<a
				...
				<a
					href={
						isElectron
							? "https://github.com/Jun-Murakami/TaskTrees-Electron"
							: "https://github.com/Jun-Murakami/TaskTrees"
					}
					target="_blank"
					rel="noreferrer"
				>
				...
				|{" "}
				<a
					href="https://jun-murakami.web.app/privacy-policy-tasktrees"
					target="_blank"
					rel="noreferrer"
				>
					Privacy policy
